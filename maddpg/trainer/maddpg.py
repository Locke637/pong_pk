...
def p_train(make_obs_ph_n, act_space_n, p_index, p_func, q_func, optimizer, grad_norm_clipping=None, local_q_func=False, num_units=64, scope="trainer", reuse=None):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        act_ph_n = [tf.placeholder(tf.float32,shape=(None,1),name="action"+ str(i)) for i in range(len(act_space_n))]
        # print(act_ph_n)
...
def q_train(make_obs_ph_n, act_space_n, q_index, q_func, optimizer, grad_norm_clipping=None, local_q_func=False, scope="trainer", reuse=None, num_units=64):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        obs_ph_n = make_obs_ph_n
        # act_ph_n = [act_pdtype_n[i].sample_placeholder([None], name="action"+str(i)) for i in range(len(act_space_n))]
        act_ph_n = [tf.placeholder(tf.float32, shape=(None, 1), name="action" + str(i)) for i in
...
class MADDPGAgentTrainer(AgentTrainer):
    def __init__(self, name, model, obs_shape_n, act_space_n, agent_index, args, local_q_func=False):
        ...
        self.q_train, self.q_update, self.q_debug = q_train(
            ...
            q_index=agent_index,
            q_func=model[0],
            optimizer=tf.train.AdamOptimizer(learning_rate=args.qlr),
        ...
        self.act, self.p_train, self.p_update, self.p_debug = p_train(
            ...
            p_func=model[1],
            q_func=model[0],
            optimizer=tf.train.AdamOptimizer(learning_rate=args.plr),
            grad_norm_clipping=0.5,
            local_q_func=local_q_func,
            num_units=args.num_units
        )
    ...
    def update(self, agents, t):
        if len(self.replay_buffer) < self.max_replay_buffer_len: # replay buffer is not large enough
            if self.max_replay_buffer_len-len(self.replay_buffer) > 0 and t%5000==0:
                print("{:.2f}".format((self.max_replay_buffer_len-len(self.replay_buffer))/self.max_replay_buffer_len*100) + "%", end="")
            return
        if not t % 1000 == 0:  # only update every 100 steps
            return
